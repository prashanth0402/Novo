...
func GmailRetriever(w http.ResponseWriter, r *http.Request) {
	...
	(w).Header().Set("Access-Control-Allow-Headers", "setDate,Accept,Content-Type,Content-Length,Accept-Encoding,X-CSRF-Token,Authorization")
	if r.Method == "GET" {
		...
		lSetDate := r.Header.Get("setDate")
		// Connect to the mail server
		log.Println("lSetDate", lSetDate)
			...
			if lErr2 != nil {
				...
				if lErr3 != nil {
					...
					if lErr4 != nil {
						...
						recentEmailUID, lErr5 := fetchEmailUIDs(c, uids, lSetDate)
						if lErr5 != nil {
							...
							if recentEmailUID != nil {
								...
								<-done
								lGmailResp.ErrMsg = "Mail Readed SucessFully"
							} else {
...
func selectInbox(c *client.Client) error {
	...
	if err != nil {
		log.Println("Error selecting INBOX:", err)
		return err
	}
...
func searchForEmails(c *client.Client, from string, keywords []string) ([]uint32, error) {
	...
	if err != nil {
		log.Println("Error searching for emails:", err)
		return nil, err
	} else {
		if len(uids) == 0 {
...
func fetchEmailUIDs(c *client.Client, uids []uint32, pSetDate string) ([]uint32, error) {
	log.Println("fetchEmailUIDs")
	...
	if pSetDate != "" {
		var lErr1 error
		parsedDate, lErr1 = time.Parse("2006-01-02", pSetDate)
		if lErr1 != nil {
			log.Println("Error parsing date:", lErr1)
			return currentDayEmailUIDs, lErr1
		}
	}
	...
	for _, uid := range uids {
		...
		if pSetDate == "" {
			...
			if emailDate.Year() == parsedDate.Year() && emailDate.Month() == parsedDate.Month() && emailDate.Day() == parsedDate.Day() {
				log.Println("parsed if")
				currentDayEmailUIDs = append(currentDayEmailUIDs, uid)
			}
		}
		// emailDate = emailDate
	}
	log.Println("currentDayEmailUIDs", currentDayEmailUIDs)
...
func setRbiMail(pbond Bond) error {
	...
	if lErr1 != nil {
		...
		defer lDb.Close()
		switch pbond.SecurityCategory {
		case "T-BILL":
			pbond.RbiName = splitTBillString(pbond.AuctionDescription)
		case "CG":
			pbond.RbiName = SplitCGString(pbond.AuctionDescription)
		case "GS":
			//  GS is Still Not properly Defined
			pbond.RbiName = splitGSstring(pbond.ISIN)
		case "SG":
			pbond.RbiName = splitSGstring(pbond.ISIN, pbond.AuctionDescription)
			// case "GG":
			// 	pbond.RbiName = SplitGGstring(pbond.AuctionDescription)
		}
...
func InsertRBiMail(pbond Bond, lDb *sql.DB) error {
	...
	( SecurityCategory, coupType, ISIN, auctionDescription, MaturityDate, allotmentDate, SettlementDate, biddingStartTime, biddingEndTime, UPIEndTime, NetBankingEndTime, NACHEndTime, IndicativeYield, createdBy, createdDate,updatedBy,updatedDate,RbiName)
	...
	_, lErr2 := lDb.Exec(lCoreString, pbond.SecurityCategory, pbond.CoupType, pbond.ISIN, pbond.AuctionDescription, pbond.MaturityDate, pbond.AllotmentDate, pbond.SettlementDate, pbond.BiddingStartTime, pbond.BiddingEndTime, pbond.UPIEndTime, pbond.NetBankingEndTime, pbond.NACHEndTime, pbond.IndicativeYield, common.AUTOBOT, common.AUTOBOT, pbond.RbiName)
...
func updateRbiMail(pbond Bond, lDb *sql.DB) error {
	...
	NACHEndTime=STR_TO_DATE(?, '%d-%b-%Y %h:%i:%s %p'), IndicativeYield=?, updatedDate= now(), updatedBy=?,RbiName = ?
	WHERE SecurityCategory=? and ISIN=?; `
	_, lErr1 := lDb.Exec(lCoreString, pbond.CoupType, pbond.AuctionDescription, pbond.MaturityDate, pbond.AllotmentDate, pbond.SettlementDate, pbond.BiddingStartTime, pbond.BiddingEndTime, pbond.UPIEndTime, pbond.NetBankingEndTime, pbond.NACHEndTime, pbond.IndicativeYield, common.AUTOBOT, pbond.RbiName, pbond.SecurityCategory, pbond.ISIN)
	if lErr1 != nil {
...
func splitTBillString(pAuctionDescription string) string {
	log.Println("splitTBillString (+)")
	RBiName := ""
	// Find the index of 'T'
	// Remove spaces
	pAuctionDescription = strings.ReplaceAll(pAuctionDescription, " ", "")

	// Remove 'D' character
	pAuctionDescription = strings.ReplaceAll(pAuctionDescription, "D", "")

	// Remove '20' substring
	pAuctionDescription = strings.ReplaceAll(pAuctionDescription, "20", "")
	pAuctionDescription = strings.ReplaceAll(pAuctionDescription, "B", "")

	if strings.HasPrefix(pAuctionDescription, "091") {
		// Remove the leading "0"
		RBiName = pAuctionDescription[1:]
	} else {
		RBiName = pAuctionDescription
	}
...
func SplitCGString(pAuctionDescription string) string {
	log.Println("splitSGString (+)")
	// Check if the string has at least 6 characters
	if strings.Contains(pAuctionDescription, "GREEN") {
		splitAuction := strings.Split(pAuctionDescription, " ")
		if len(splitAuction) >= 5 {
			pAuctionDescription = splitAuction[3] + splitAuction[5]
		}
	} else {
		pAuctionDescription = strings.ReplaceAll(pAuctionDescription, " ", "")
		pAuctionDescription = strings.ReplaceAll(pAuctionDescription, ".", "")
		pAuctionDescription = strings.Replace(pAuctionDescription, "0", "", 1) // pAuctionDescription = strings.ReplaceAll(pAuctionDescription, "20", "")
		if strings.Contains(pAuctionDescription, "GOVTSTOCK") {
			pAuctionDescription = strings.ReplaceAll(pAuctionDescription, "GOVTSTOCK", "GS")
		}
	}

	log.Println("splitSGString (-)")
	return pAuctionDescription
}

func splitGSstring(Input string) string {
	log.Println("splitGSstring (+)")
...
func splitSGstring(pIsin string, pAuction string) string {
	log.Println("splitSGstring (+)")
	RbiName := ""
	if len(pIsin) > 0 {
		// Remove the last character
		splitAuction := strings.Split(pAuction, " ")
		RbiName = pIsin[4:6] + splitAuction[len(splitAuction)-1]
	}
	log.Println("splitSGstring (+)")
...
func SplitGGstring(pAuction string) string {
	split1 := ""
	splitAuction := strings.Split(pAuction, " ")
	if strings.Contains(strings.ToUpper(pAuction), "GR") {
		split1 = "GREEN" + splitAuction[len(splitAuction)-1]
	}
	return split1
}
