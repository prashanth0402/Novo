    ...
    <div>
        ...
        <v-layout row wrap class="mb-5 mt-3" style="padding-left: 12px;">
            <div class="d-flex align-center">
                <v-img src="https://flattrade.s3.ap-south-1.amazonaws.com/promo/SdlLogo.png" height="20" width="25" contain
                    class="d-flex"></v-img>
                <span class="font-weight-bold" style="margin-left: 13px;">SDL -State Development Loans</span>
            </div>
        </v-layout>

        <!-- values -->
        <v-layout class="my-10" v-if="sdldynamicItem.length == 0">
            <v-slide-x-transition mode="out-in" appear>
                <v-flex class="ml-5 text text--disabled">
                    <!-- <p v-if="loansresult.length == 0 && Flag == 'I'">No SDLs are open for sale currently.</p>
                    <p v-if="loanshistory.length == 0 && Flag == 'O'">You haven't invested in any SDLs.</p> -->
                    <p>{{ sdldynamicText }}</p>
                </v-flex>
            </v-slide-x-transition>
        </v-layout>

        <v-card class="elevation-0" v-if="sdldynamicItem.length != 0">
            <v-card-title>
            ...
            <v-slide-x-transition mode="out-in" appear>
                <v-data-table :headers="sdldynamicHeader" :items="sdldynamicItem" :search="search" :items-per-page="10"
                    :key="tableKey"
                    ...
                    <template v-slot:item.symbol="{ item }" v-if="loansresult != [] && Flag == 'I'">
...
export default {
    ...
    computed: {
        ...
        sdldynamicItem: {
            get() {
                if (this.Flag == "I" && this.sdlmasterFound == "Y") {
                    return this.loansresult
                } else if (this.Flag == "O" && this.sdlhistoryFound == "Y") {
                    return this.loanshistory
                } else {
                    return []
                }
            }
    ...
    props: {
        ...
        // btnText: String,
        // Modbtn: Boolean,
        sdldynamicText: String,
        sdlmasterFound: String,
        sdlhistoryFound: String,
    },
    components: {
        detailCard
    },
